...
```bash
harness/corpus.sh              # the whole regression corpus (all effects), one PASS/FAIL table
...
## Corpus runner (Phase 6)

`corpus.py` (wrapped by `corpus.sh`) drives every per-effect harness below in one
command and prints a PASS/FAIL summary: determinism/non-perturbation, left-border
removal, raster-bar round-trip, intra-line vertical bands and the STE scroller.
The checks run sequentially — each launches its own headless Hatari on the fixed
remote port. Exit 0 = whole corpus passed. `--only <substr>` runs a subset. The
scroller check uses `tests/effects/corpus-scroller.s`, a canonical message
scroller generated by the current codegen.
