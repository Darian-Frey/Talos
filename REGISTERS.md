...
- **F-220** Whole-frame register-write timeline — a *Register timeline* tab that captures every write to the video/palette/sync/scroll register block (`$ff8200-$ff82ff`) across one frame and plots each on the (cycle, scanline) beam grid, colour-coded by register class (palette / sync / resolution / STE-scroll / line-width / video-base / other) with a per-class legend and counts. **Built (Phase 6):** backed by the opt-in `regtrace` B2 tap (`patches/0004-*`) that records each write with its `Video_GetPosition` beam position into a host-side ring; the client's `RegTraceController` captures a frame (run to VBL → `regtrace on` → run one frame → dump → `regtrace off`) and `RegTimelineView` renders it. This is the built answer to the C-004 "does a per-frame event stream fit over the socket" question — it does, with three orders of magnitude of headroom (see C-004, D-004).
...
- **C-004** Socket bandwidth for per-cycle event streams was the measured risk of Phase 1 and the trigger for the D-004 reversal (single-process embedding). **Measured & resolved 2026-07-21 in the socket's favour (F-220, `patches/0004-*`).** A whole-frame register-write dump for a busy frame (multi-split Spectrum-512-lite) is ~6260 writes / ~104–166 KB, cleared over the localhost socket in ~0.1 ms; the pathological continuous 50 fps worst case (~10k writes/frame) is ~8.5 MB/s = **0.68 %** of a measured ~1.2 GB/s localhost TCP ceiling. The only cost worth watching is the client-side parse, and the natural model is capture-per-frame-and-scrub, not a continuous stream. The D-004 reversal is **not** needed for per-frame event data. (A literal *per-cycle* stream of every bus event at speed remains untested and is not required by any built feature; if one is ever needed, re-measure at that volume.)
