...
- **MFP timer / interrupt visualisation** — ✓ **Built.** The *MFP* tab reads the 68901 register block (`$fffa00`–`$fffa2f`) from the live machine and decodes it: a timer table (A–D: mode, prescaler, data, frequency = XTAL ÷ prescaler ÷ data, interrupts/frame ≈ beam interval) and an interrupt matrix (enabled / unmasked / pending / in-service, timer rows highlighted). Constants sourced from Hatari `mfp.c`/`clocks_timings.c` (C-007); validated against a live read (Timer-C = 200 Hz). Talos reads the MFP, never emulates it.
...
- **Regression-corpus runner** — ✓ **Built.** `harness/corpus.sh` (→ `corpus.py`) runs the whole corpus in one command — determinism/non-perturbation (D-009), left-border removal, raster-bar round-trip, intra-line vertical bands and the STE scroller — driving each per-effect harness in turn (sequential, since each grabs the fixed remote port), printing a PASS/FAIL table and exiting non-zero on any failure. Validated 5/5 (~70 s); ships with a canonical `tests/effects/corpus-scroller.s` generated by the current codegen.
